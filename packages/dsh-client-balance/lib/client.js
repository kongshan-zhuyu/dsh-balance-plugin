window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SettingsSection() {
      ...
      const blankForm = { id: "", name: "", endpoint: "", responsePath: "$.data.balance", currency: "CNY", apiKey: "", credentialRef: "", route: "" };
      ...
      const saveProvider = async (event) => { event.preventDefault(); try { const data = await api("/provider", { method: "POST", body: JSON.stringify({ ...form, apiKey: form.apiKey || undefined }) }); let bindings = { ...(config.bindings || {}) }; if (form.route) { for (const [key, bid] of Object.entries(bindings)) if (bid === data.provider.id) delete bindings[key]; bindings[form.route] = data.provider.id; await api("/preferences", { method: "POST", body: JSON.stringify({ statusBar: config.statusBar, bindings }) }); } setConfig({ ...config, providers: [...config.providers.filter(item => item.id !== data.provider.id), data.provider], bindings }); setForm(blankForm); setEditing(false); setMessage(form.route ? `已接入并绑定到 ${form.route}` : form.credentialRef ? "供应商已保存；将复用模型页的凭据" : "供应商已保存；密钥已写入系统钥匙串"); loadSummary(); refreshBar(); } catch (error) { setMessage(error.message); setMessageKind("error"); } };
      const beginAdd = (source) => { setForm(source ? { ...blankForm, id: source.id.replace(/[^a-z0-9_-]/gi, "-").slice(0, 64), name: source.name, credentialRef: source.credentialRef, route: source.id } : blankForm); setEditing(true); };
      const beginPreset = (source) => { setForm({ ...blankForm, id: source.id.replace(/[^a-z0-9_-]/gi, "-").slice(0, 64), name: source.name, credentialRef: source.credentialRef, preset: "deepseek", route: source.id }); setEditing(true); };
      const beginEdit = (provider) => { setForm({ ...provider, apiKey: "" }); setEditing(true); };
