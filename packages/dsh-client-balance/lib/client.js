window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const formatMoney = (value, currency) => new Intl.NumberFormat("zh-CN", { style: "currency", currency: currency || "CNY", currencyDisplay: "narrowSymbol", maximumFractionDigits: 2 }).format(value);
...
.db-plugin-card{border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-layer-3);list-style:none;transition:border-color .16s,background .16s}
    ...
    function renderBar(config, providers) {
      ...
      const bar = ensureBar(); const selected = providers[0]; const summary = bar.querySelector(".dsh-balance-summary");; state.provider = selected || null; summary.replaceChildren(); summary.className = "dsh-balance-summary";
    ...
    function SettingsSection() {
      ...
      const blankForm = { id: "", name: "", endpoint: "", responsePath: "$.data.balance", currency: "CNY", apiKey: "", credentialRef: "", route: "", method: "GET", headersText: "", endpointBase: "", timeoutSeconds: 10, queryIntervalMinutes: 30, conversionEnabled: false, valueDivisor: 1 };
      ...
      const providerCard = (provider, modelProvider) => { const editingKey = modelProvider?.id || boundRoute(provider.id) || provider.id; return h("div", { className: "db-provider-row db-provider-card", key: modelProvider ? `model-${modelProvider.id}` : provider.id }, h("div", { className: "db-row-line" }, h("span", { className: "db-provider-name" }, modelProvider?.name || provider.name), h("span", { className: statuses[provider.id] && statuses[provider.id].status !== "ok" ? "db-live error" : "db-live" }), h("span", { className: "db-tag" }, "已开启查询"), (provider.preset === "deepseek" || provider.preset === "opencode-go") && h("span", { className: "db-tag" }, "官方内置"), !modelProvider && modelProviders.length > 0 && h("select", { className: "db-bind", value: boundRoute(provider.id), onChange: event => bindRoute(provider.id, event.target.value), title: "绑定到此模型供应商后，状态栏随所选模型显示该余额" }, h("option", { value: "" }, "未绑定模型"), modelProviders.map(mp => h("option", { key: mp.id, value: mp.id }, mp.name))), h("div", { className: "db-spacer" }), h("button", { className: "db-quiet", type: "button", onClick: () => beginEdit(provider) }, "编辑"), h("button", { className: "db-delete", type: "button", onClick: () => remove(provider.id) }, "删除")), h("div", { className: "db-row-meta" }, ...(balanceMeta(provider.id) || [])), editing === editingKey && inlineEditor()); };
