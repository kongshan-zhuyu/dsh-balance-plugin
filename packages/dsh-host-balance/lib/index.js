...
async function query(provider, credentials, force = false) {
  // 每个供应商独立控制自动查询间隔；手动刷新通过 force 跳过缓存。
  const cacheMs = Math.max(0, Number(provider.queryIntervalMinutes ?? DEFAULT_QUERY_INTERVAL_MINUTES)) * 60_000;
  const existing = cache.get(provider.id); if (!force && cacheMs > 0 && existing && Date.now() - existing.at < cacheMs) return existing.value;
  const inherited = provider.credentialRef ? await credentials.resolve(provider.credentialRef) : undefined;
  const secret = inherited?.value || await secrets.get(provider.id); if (!secret) throw new HttpError(502, "credential is missing in DSH credentials or the system keychain");
  ...
  if (response.status < 200 || response.status >= 300) throw new HttpError(502, `provider returned HTTP ${response.status}`);
...
export function apply(ctx) {
  const registration = {
    ...
    try { const url = new URL(req.url || "/", "http://local"); const config = await loadConfig();
      ...
      if (req.method === "POST" && url.pathname === "/dsh-balance/provider") { const input=await body(req); const provider=await validateProvider(input); if(typeof input.apiKey === "string" && input.apiKey.length > 0) await secrets.set(provider.id,input.apiKey); await mutateConfig(async current => { current.providers=[...current.providers.filter(p=>p.id!==provider.id),provider]; await saveConfig(current); }); cache.delete(provider.id); return json(res,200,{ok:true,provider:redactProvider(provider)}); }
      ...
      if (req.method === "DELETE" && url.pathname.startsWith("/dsh-balance/provider/")) { const id=decodeURIComponent(url.pathname.slice("/dsh-balance/provider/".length)); if(!isId(id)) return json(res,400,{ok:false,error:"invalid id"}); await mutateConfig(async current => { current.providers=current.providers.filter(p=>p.id!==id); current.bindings=Object.fromEntries(Object.entries(current.bindings || {}).filter(([,providerId])=>providerId!==id)); await saveConfig(current); }); await secrets.remove(id); cache.delete(id); return json(res,200,{ok:true}); }
